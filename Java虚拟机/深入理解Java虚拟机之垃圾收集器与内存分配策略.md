...
​	在HotSpot虚拟机里是通过写屏障（Write Barrier）技术维护卡表状态的。先请读者注意将这里提到的“写屏障”，以及后面在低延迟收集器中会提到的“读屏障”与解决并发乱序执行问题中的“内存屏障”  区分开来，避免混淆。**写屏障可以看作在虚拟机层面对“引用类型字段赋值”这个动作的AOP切面 ，在引用对象赋值时会产生一个环形（Around）通知，供程序执行额外的动作，也就是说赋值的前后都在写屏障的覆盖范畴内。**在赋值前的部分的写屏障叫作**写前屏障**（Pre-Write Barrier），在赋值后的则叫作**写后屏障**（Post-Write Barrier）。HotSpot虚拟机的许多收集器中都有使用到写屏障，但直至G1收集器出现之前，其他收集器都只用到了写后屏障。下面这段代码是一段更新卡表状态的简化逻辑：
...
​	除了写屏障的开销外，卡表在高并发场景下还面临着“伪共享”（False Sharing）问题。**伪共享**是处理并发底层细节时一种经常需要考虑的问题，现代中央处理器的缓存系统中是以缓存行（Cache Line）为单位存储的，当多线程修改互相独立的变量时，如果这些变量恰好共享同一个缓存行，就会彼此影响（写回、无效化或者同步）而导致性能降低，这就是伪共享问题。（就像是隐式的共享变量一样？）
...
​	在并发的可达性分析算法中，虽然解决了停顿问题，但却无法保证一致性的问题。并发可达性分析算法其实就是标记对象的过程与用户线程并发执行，为了详细说明并发可达性分析算法存在的问题，将可达性分析算法中对象的状态分成以下三种：

- **未标记状态**：表示该对象尚未被垃圾收集器访问，该状态可以转换成**正在访问状态**。如果分析结束后仍然处于该状态，则表明该对象不可达。
- **正在访问状态**：表示对象已经被访问过，但该对象上至少存在一个引用还没有被扫描过（为了标记下一个对象）。该状态可以转换成**已标记状态**。
- **已标记状态**：表示该对象已经被访问过，并且该对象所有引用都被扫描过。该状态表示对象是存活的。该状态对象的下一个需要标记的对象（引用链上的下一个节点）不可能是未标记状态[^注4.6.1]。**该状态的对象不会被二次扫描或标记**。



​	可达性分析算法存在两种问题：**原本消亡的对象被错误地标记为存活** 和 **原本存活的对象被错误地标记为消亡**。

- **原本消亡的对象被错误地标记为存活**：

    ​	这种问题的发生原因是：原本是已标记状态的对象，在与用户线程并发执行的情况下，用户线程修改了对象图的结构，导致该对象在标记后应该是需要回收的对象。对于这种问题，其实也是可以容忍，因为该对象可以在下一次垃圾收集进行回收，这种对象也被称为**浮动垃圾**。

- **原本存活的对象被错误地标记为消亡**：

    ​	这种问题不同于上一种，将存活对象标记为消亡对象，这将可能导致系统崩溃，是并发可达性分析算法需要着重解决的问题。有理论证明，当且仅当以下两种条件同时满足时，就会产生该问题：

    - 赋值器插入了一条或多条从**已标记状态对象**到**未标记状态对象**的新引用；
    - 赋值器删除了全部从**正在访问状态对象**到该**未标记状态对象**的直接或间接引用；

​			因此，要解决该问题就需要破坏这两个条件中的任意一种。由此分别产生了两种解决方案：**增量更新**和**原始快照**。

- **增量更新**：

    ​	破坏第一个条件，当**已标记状态对象**插入新的**未标记状态对象**的引用关系时，就将这个插入的引用记录下来，等并发扫描之后，再将这些记录过的引用关系中的**已标记状态对象**为根节点，重新扫描一次。可以理解为新增引用的**已标记状态**对象会在并发扫描后变回**正在访问状态**。

- **原始快照**：

    ​	破坏第二个条件，当**正在访问状态对象**删除指向**未标记状态对象**的引用关系时，记录下这个要删除的引用，在并发结束后，再将这些记录过的引用关系中的**正在访问状态对象**作为根节点，重新扫描一次。可以理解为不管时候删除，都会按照一开始时的对象图快照进行扫描，只不过将扫描延后了。
...
​	**个人问题**：两个条件中，只要满足其中一个条件也会产生存活变消亡的问题。在**已标记状态对象**中直接引用新对象，这样除了重新扫描**已标记状态对象**，否则新对象将会被回收。

​	**答**：使用增量更新的并发收集可能不会存在该问题，但使用原始快照的并发收集会出现该问题。但使用原始快照的收集器对于在并发收集中产生新对象都记为存活对象，因此也不会出现这种问题。而对于引用原本消亡的对象是不可能，因为原本消亡的对象是不可达，即不存在引用。



[^注4.6.1]:因为当对象状态为已标记状态时（表示对象可达），其引用的对象也必定是可达的
...
- ​	**对于回收过程中新创建对象的内存分配问题**，G1为每一个Region设计了两个名为**TAMS**（Top at Mark Start）的指针，把Region中的一部分空间划分出来用于并发回收过程中的新对象分配，并发回收时**新分配的对象地址都必须要在这两个指针位置以上**。G1收集器默认在这个地址以上的对象是被隐式标记过的，即**默认它们是存活的**，不纳入回收范围。与CMS中的“Concurrent Mode Failure”失败会导致Full GC类似，如果内存回收的速度赶不上内存分配的速度，G1收集器也要被迫冻结用户线程执行，导致Full GC而产生长时间“Stop The World”。
...
![空间分配担保](C:\Users\Administrator\Desktop\学习\java\Java-learn\Java虚拟机\img\空间分配担保.png)
...
《深入理解Java虚拟机：Java高级特性与最佳实践》第三版（该书的第3章）——周志明著

...
​	Parallel Old收集器是PS收集器的老年代版本，同样是并行收集器，收集算法采用的是标记-整理算法实现。该收集器是在 JDK1.6时才开始提供的。直到Parallel Old收集器出现后，“吞吐量优先”收集器终于有了比较名副其实的搭配组合，在注重吞吐量或者处理器资源较为稀缺的场合，都可以优先考虑PS+Parallel Old收集器这个组合。

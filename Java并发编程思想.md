...
​	对象的组合是指将现有的线程安全的组件组合为更大规模的组件或程序。可以通过一些组合模式进行组合，使一个类更容易成为线程安全的。
...
- 找出构成对象状态的所有变量；
- 找出约束状态变量的不可变性条件；
- 建立对象状态的并发访问管理策略（同步策略）；

​	状态分为可变状态和不可变状态，约束状态变量的不可变性条件一般指对状态进行修改的行为或涉及变量状态的操作。同步策略规定了如何将不可变性、线程封闭与加锁机制等结合起来以维护线程的安全性，并规定了哪些变量由哪些锁来保护。

​	设计线程安全的类主要与状态有关，应当着重注意涉及状态的操作。对一些产生状态变化的操作（不变性要求及后验条件[^注3.1.1]，如修改状态的复合操作）、依赖状态的操作（先验条件，如对一个队列操作前的非空判断）都应着重注意，需要判断这些操作是否需要同步。

[^注3.1.1]:后验条件可以理解为修改状态后，判断该状态是否是无效状态。无效状态也可以理解为过期状态。

## 实例封闭

​	通过封装实现实例封闭，能够简化线程安全类的实现过程。封装限制了状态的访问路径，如果与合适的加锁策略结合起来，就可以确保以线程安全的方式来使用非线程安全的对象。
